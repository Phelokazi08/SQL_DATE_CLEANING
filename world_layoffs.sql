...
('Byjus',	'Bengaluru',	'Education',	1500,	NULL,	'2/2/2023' , 'Private Equity',	'India',	5500),
...
('Ballys Interactive',	'Providence',	'NULL',	NULL,	0.15,	'1/18/2023',	'Post-IPO',	'United States',	946),
...
('Byjus',	'Bengaluru',	'Education',	2500,	0.05,	'10/12/2022',	'Private Equity',	'India',	5500),
...
('OReilly Media',	'SF Bay Area',	'Media',	75,	0.15,	'3/25/2020',	'Unknown',	'United States',	NULL),
...
--COPY THE ORIGINALA TABLE

SELECT *
INTO layoffs_cp -- NAME the NEW TABLE
FROM layoffs_org -- NAME OF the OLD TABLE

--////////////////////////////////////////////////////////////////////////////////////

 --     1 REMOVE DUPLICATE using the COPY TABLE "layoffs_cp"

    -- CODE OF CHECK DUPLICATE
SELECT *, 
ROW_NUMBER() OVER( PARTITION BY company, industry, total_laid_off, percentage_laid_off, [date] ORDER BY [date])
FROM layoffs_cp;

         -- CODE TO SEE ONLY DUPLICATES (use CTE)
WITH duplicate_cte AS
(
   SELECT *, ROW_NUMBER()
       OVER( PARTITION BY company, [location], industry, total_laid_off, percentage_laid_off, [date],
	   stage, country, funds_raised_millions ORDER BY [date]) AS row_d
   FROM layoffs_cp
)
...
-- Replace the first code

DELETE
FROM duplicate_cte
WHERE row_d > 1; -- MAIN PART
...
-- Remove leading and trailing spaces from a column in a table (TRIM)

SElECT company, TRIM(company)
FROM layoffs_cp; -- just see difference

--UPDATE
UPDATE layoffs_cp
SET company = TRIM(company); -- MAIN PART

 
-- Do this code when we have same name but some have extra characters 
-- e.g ('name.' , 'name' and 'name_of_name') then we update all name like to be 'name'
SELECT industry
FROM layoffs_cp
...
UPDATE layoffs_cp
SET industry = 'Crypto'
WHERE industry LIKE 'Crypto%' -- MIAN PART 
...
WHERE country LIKE 'United State%' -- MIAN PART 

-- OR WE CAN USE (TRIM(TRAILING...)) WHEN THE IS NO MUCH CHARACTERS
--e.g ('name.' , 'name') 
-- ( UPDATE table_name
--   SET column_name = TRIM(TRAILING '.' FROM column_name)
--   WHERE column_name LIKE 'name%'); MAIN PART

--///////////////////////////////////////////////////////////////////////////

  --     3 NULL VALUE OR BLANCK SPACE

SELECT industry
FROM layoffs_cp
WHERE industry = '' OR industry is NULL -- JUST CHECK

-- This will help to identify the rows where the industry column should be updated.
...
-- Update the industry field using a self-join
UPDATE t1
...
WHERE t1.industry = ' ' or t1.industry is NULL
  AND t2.industry IS NOT NULL; --MAIN PART

--Here, I use this update because the industry column for the Airbnb company does not show any effect from the above update.
UPDATE layoffs_cp
set industry = 'Travel'
...
--/////          Next, let's explore data visualization using other tools such as Power BI         /////
